...
import { MOCK_VEHICLES, MOCK_LOCATIONS, MOCK_LOC_MAP } from '@/lib/mock'
import type { Vehicle, VehicleType } from '@/types'

export default function HubVehicles() {
  const toast = useToast()
  const [vehicles, setVehicles]   = useState<Vehicle[]>(MOCK_VEHICLES.filter((v) => v.location_id === 'hub-hfd' || v.location_id === 'hub-ens'))
  ...
  const assignAll = () => {
    ...
    toast(`${selected.length} voertuig${selected.length !== 1 ? 'en' : ''} toegewezen aan ${MOCK_LOC_MAP[targetLoc]?.name}.`)
    setSelected([])
    setTargetLoc('')
  }

  const destLocations = MOCK_LOCATIONS.filter((l) => !l.is_hub)
    ...
    <div>
      ...
      <div className="htf-card" style={{ padding: 0 }}>
        <table className="data-table">
          <thead>
            <tr>
              <th style={{ width: 40 }}><input type="checkbox" onChange={(e) => setSelected(e.target.checked ? filtered.map((v) => v.id) : [])} checked={selected.length === filtered.length && filtered.length > 0} /></th>
              <th>ID</th>
              <th>Type</th>
              <th>Status</th>
              <th>Hub</th>
          ...
          <tbody>
            {filtered.map((v) => (
              <tr key={v.id} style={{ background: selected.includes(v.id) ? 'var(--cream2)' : 'transparent' }}>
                ...
                <td style={{ fontSize: 12, color: 'var(--muted)' }}>{MOCK_LOC_MAP[v.location_id]?.name}</td>
                <td>
                  <button
